...
namespace
{
	...
	FString NormalizeLuaName(FString Name)
	{
		Name.erase(
			std::remove_if(
				Name.begin(),
	...
	const char* PawnFacingModeToString(EPawnFacingMode Mode)
	{
		switch (Mode)
		{
	...
	EPawnFacingMode PawnFacingModeFromString(const FString& Value)
	{
		const FString Normalized = NormalizeLuaName(Value);
		if (Normalized == "none")
		{
			return EPawnFacingMode::None;
		}
		if (Normalized == "controlrotationyaw" || Normalized == "controlrotation")
		{
			return EPawnFacingMode::ControlRotationYaw;
		}
		if (Normalized == "movementinputdirection" || Normalized == "movementinput")
		{
			return EPawnFacingMode::MovementInputDirection;
		}
		if (Normalized == "movementvelocitydirection" || Normalized == "movementvelocity" || Normalized == "velocity")
		{
			return EPawnFacingMode::MovementVelocityDirection;
		}
		if (Normalized == "customworlddirection" || Normalized == "custom")
		{
...
void RegisterActorComponentBinding(sol::state& Lua)
{
	Lua.new_usertype<FLuaActorComponentHandle>(
		...
		{
			sol::state_view LuaView(State);
			UPawnOrientationComponent* Orientation = Cast<UPawnOrientationComponent>(Self.Resolve());
			if (!Orientation)
			{
				return sol::nil;
			}
			FLuaPawnOrientationComponentHandle Handle;
			Handle.UUID = Orientation->GetUUID();
			return sol::make_object(LuaView, Handle);
		}
	);
}
...
void RegisterPawnOrientationComponentBinding(sol::state& Lua)
{
	Lua.new_usertype<FLuaPawnOrientationComponentHandle>(
		...
		"Owner",
		sol::property(
			[](const FLuaPawnOrientationComponentHandle& Self)
			{
		...
		sol::property(
			...
			{
				UPawnOrientationComponent* Component = Self.Resolve();
				return Component ? PawnFacingModeToString(Component->GetFacingMode()) : FString("None");
			},
			[](const FLuaPawnOrientationComponentHandle& Self, const FString& Value)
			{
				if (UPawnOrientationComponent* Component = Self.Resolve())
				{
					Component->SetFacingMode(PawnFacingModeFromString(Value));
				}
			}
		),
		...
		sol::property(
			...
			[](const FLuaPawnOrientationComponentHandle& Self, int32 Value)
			{
				if (UPawnOrientationComponent* Component = Self.Resolve())
				{
					Component->SetFacingMode(static_cast<EPawnFacingMode>(Value));
				}
			}
		),
		...
		sol::property(
			...
			{
				UPawnOrientationComponent* Component = Self.Resolve();
				return Component ? Component->GetCustomFacingDirection() : FVector::ZeroVector;
			},
		...
		"ListProperties",
		[](const FLuaPawnOrientationComponentHandle& Self, sol::this_state State)
		{
			return FLuaPropertyBridge::ListProperties(State, Self.Resolve());
		},

		"GetProperty",
		[](const FLuaPawnOrientationComponentHandle& Self, const FString& Name, sol::this_state State)
		{
			return FLuaPropertyBridge::GetProperty(State, Self.Resolve(), Name);
		},

		"SetProperty",
		[](const FLuaPawnOrientationComponentHandle& Self, const FString& Name, sol::object Value)
		{
			return FLuaPropertyBridge::SetProperty(Self.Resolve(), Name, Value);
		}
	);
