...
AActor* FObjectPoolSystem::SpawnActorForPool(UWorld* World, const FPoolKey& Key, bool bStartInactive)
{
	...
	if (Key.IsPrefab())
	{
		UPrefab* Prefab = FResourceManager::Get().LoadPrefab(Key.PrefabKey);
		if (!Prefab || !Prefab->GetBaseActorClass())
		{
			return nullptr;
		}

		Object = FObjectFactory::Get().Create(Prefab->GetBaseActorClass()->GetName(), World);
	}
	else
	{
		Object = FObjectFactory::Get().Create(Key.ClassKey->GetName(), World);
	}
	...
	{
		UPrefab* Prefab = FResourceManager::Get().LoadPrefab(Key.PrefabKey);
		if (!Prefab || !FSceneSaveManager::ApplyPrefabDataToActor(Actor, Prefab->GetPrefabData()))
		{
...
void FObjectPoolSystem::ResetActorFromPrefab(AActor* Actor, const FPoolKey& Key)
{
	...
	if (Prefab)
	{
		FSceneSaveManager::ApplyPrefabDataToActor(Actor, Prefab->GetPrefabData());
	}
}
